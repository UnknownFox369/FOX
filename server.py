...
def send_data():
    ...
    while True:
        ...
        data = bytes(input(f"{Fore.RED}command: {Style.RESET_ALL}"), "utf-8")
    ...
    while True:
        ...
        for index, client in enumerate(clients):
            try:
                ...
                data = data.replace("ÿ", " ")
                if len(data) > 0:
                    print(Fore.CYAN , client["address"] , Style.RESET_ALL," : ", data, "\n")
            except:
                print(Fore.CYAN , client["address"] , Style.RESET_ALL, "disconnected \n")
                clients.pop(index)
